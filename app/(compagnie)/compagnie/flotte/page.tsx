...
import { 
  Car, Plus, ArrowLeft, Search, Info, CheckCircle, Truck, Bus
} from 'lucide-react';
...
export default function FlottePage() {
  ...
  const loadVehicles = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/compagnie/vehicules');
      if (res.status === 401) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(editingId ? `/api/compagnie/vehicules/${editingId}` : '/api/compagnie/vehicules', {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/compagnie/vehicules/${id}`, {
        method: 'DELETE',
  ...
  const closeModal = () => {
    ...
    setEditingId(null);
    setFormData({ plaque: '', marque: '', modele: '', type: 'BUS', nombrePlaces: '' });
  };
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg focus:ring-2 focus:ring-blue-500 text-white uppercase placeholder:normal-case placeholder:text-white/30"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 bg-slate-800 border border-white/10 rounded-lg focus:ring-2 focus:ring-blue-500 text-white"
                >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg focus:ring-2 focus:ring-blue-500 text-white"
                  />
                </div>
                <div>
