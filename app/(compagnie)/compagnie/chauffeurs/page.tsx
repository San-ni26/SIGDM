...
export default function ChauffeursPage() {
  ...
  const loadChauffeurs = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/compagnie/chauffeurs');
      if (res.status === 401) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/compagnie/chauffeurs/${id}`, {
        method: 'DELETE',
      ...
      <main className="max-w-6xl mx-auto px-4 mt-8 space-y-6">
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-white/60 mt-1">
              Pour ajouter un chauffeur à votre compagnie, vous devez connaître son matricule citoyen à 5 caractères (ex: A3B7K). Le chauffeur doit d'abord s'inscrire sur l'application citoyen.
            </p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white/5 border border-white/10 rounded-2xl">
            ...
            <p className="text-white/50 text-sm mb-6">Vous n'avez pas encore ajouté de chauffeur à votre compagnie.</p>
            <button 
              onClick={() => setShowAddModal(true)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredChauffeurs.map((employe) => (
              <div key={employe.id} className="bg-white/5 border border-white/10 rounded-2xl p-5 hover:bg-white/10 transition-colors flex flex-col sm:flex-row gap-4">
                ...
                <div className="w-16 h-16 bg-gradient-to-br from-blue-500/20 to-indigo-500/20 rounded-full flex items-center justify-center shrink-0 border border-white/10">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-3 py-3 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-blue-500 text-white font-mono text-lg tracking-widest uppercase placeholder:normal-case placeholder:tracking-normal placeholder:text-white/30"
                  />
                </div>
